...
namespace tls {
    ...
    // Data written to the replicate is copied to all threads that read
    ...
    class replicate {
        ...
        struct thread_data {
            ...
            iterator find(replicate<T>& instance) {
            ...
            void invalidate(replicate<T>& instance) {
                auto const it = find(instance);
                it->invalidated = true;
            }

            // Returns the local data
            T const& read(replicate<T>& instance) {
                auto const it = find(instance);
                update_data(*it);
                return it->data_copy;
            }
            ...
            template<class Fn>
            auto read(replicate<T>& instance, Fn f) {
                auto const it = find(instance);
                update_data(*it);
    ...
    private:
        // the head of threads that access this instance. Changes to 'data'
